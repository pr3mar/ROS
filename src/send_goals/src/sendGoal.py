...
class  SendGoal():
	def detection_thresh(self, points):
		for newPoint in points.markers:
			...
			if min_point == None:
				self.det[str(xp)+";"+str(yp)+";"+str(zp)] = {'count':1, 'detected':False, 'point':str(xp)+";"+str(yp)+";"+str(zp)}
			elif min_point != "not":
				min_point['count'] += 1
	...
	def callback(self, data):
        ...
        def __init__(self):
            ...
            x_coord = [3.702, 3.748,3.748,3.748, 2.332, 0.887, 0.295, -0.029, -0.164, -0.554, -0.752, 0.637, 0.856, 1.029, 2.498]
            y_coord = [0.137, -0.362,-0.362,-0.362, -0.044, -0.171, -0.004, -1.000, -2.184, -3.065, -4.190, -3.298, -2.084, -1.081, -0.037]
            z_coord = [0.649, -0.065,-0.726,0.686, 0.649, 0.649, 0.984, 0.979, 0.996, 0.984, -0.820, -0.731, -0.078, -0.078, -0.734]
            w_coord = [0.760, 0.997, 0.686, 0.726, 0.76, 0.760, 0.176, 0.199, 0.078, 0.176, 0.571, 0.681, 0.996, 0.996, 0.678]
            self.point_list = []
